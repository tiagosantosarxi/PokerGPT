...
class ReadPokerTable:
    ...
    def find_player_cards(self, player_number):
        ...
        player_card_positions = {
            1: [(0.437, 0.621), (0.501, 0.621)],  # Player 1
        ...
        for index, (num_x, num_y) in enumerate(player_card_positions[player_number], start=1):
            icon_x, icon_y = num_x, num_y + 0.033  # Adjust for icon position
            ...
            image_path = 'tmp\%s' % card_filename

            # Save the grayscale image
            # cv2.imwrite(image_path, screenshot_icon_gray)
            ...
            image_path = 'tmp\%s' % card2_filename

            # Save the grayscale image
            # cv2.imwrite(image_path, screenshot_num_gray)
            # i+=1
                    ...
                    if current_cards_detected_length == 2:
                        ...
                        with self.game_state_lock:
                            ...
                            if new_cards_detected != current_cards_stored:
                                print(
                                    f"Player{player_number} NEW card = {new_cards_detected} | current_cards_stored = {current_cards_stored}")
